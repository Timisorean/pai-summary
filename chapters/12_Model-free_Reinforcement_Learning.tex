...
\vspace{0.5mm}\hrule\vspace{0.5mm}
\includegraphics[width=0.7\linewidth,trim={0 0.8cm 1.5cm 0.6cm},clip]{images/Online_actor_critic.png}
\begin{tikzpicture}[baseline, overlay]
  \node[anchor=west, font=\fontsize{6pt}{6pt}\selectfont, anchor=base, xshift=-0.6cm, yshift=2cm, fill=yellow!10] at (0, 0) {
...
{\fontsize{6}{6}\selectfont 
    ...
    reward modeling or RL loops. \hl{(offline, model-free, ref.-based)}\\[-0.5ex]
    \scalebox{0.7}{\textbullet} \textbf{PETS:} combines probabilistic ensembles to capture un-\\[-0.5ex]
    certainty with MPC for planning. \hl{(model-based, stochastic, }\\[-0.5ex]
    \hl{MPC)}
    \scalebox{0.7}{\textbullet} \textbf{UCRL:} implements optimistic exploration via\\[-0.5ex]
    plausible models to min. regret. \hl{(model-based, exploration,}\\[-0.5ex]
    \hl{opt. i.t.f.o. uc)}
    \scalebox{0.7}{\textbullet} \textbf{H-UCRL:} extends UCRL to hierarchical\\[-0.5ex]
    structures to manage exploration. \hl{(model-based, opt. i.t.f.o. uc)}
}
