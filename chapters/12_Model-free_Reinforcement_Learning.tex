...
In tab. methods: Storing val. fn., need at least $O(|\mathcal X|)$ space. Storing Q-fn, even need $O(|\mathcal X| \cdot |\mathcal A|)$ space.\\
Time req. to compute value fn. for every state-action pair exactly grows poly. in size of state-action space.\\
...
\scalebox{0.7}{\textbullet} \textbf{DDPG:} Det. grads. enable cont. control with target networks \hl{(off-pol., m-free, AC)}
\scalebox{0.7}{\textbullet}~\textbf{SAC:} Max. entropy obj. balances exploration and performance \hl{(off-pol., m-free)}
\scalebox{0.7}{\textbullet}~\textbf{DPO:} Bypasses explicit reward modeling \hl{(offline, m-free, ref.-based)}
\scalebox{0.7}{\textbullet}~\textbf{PETS:} Ensembles plus MPC handle uncertainty in planning. \hl{(m-based)}
