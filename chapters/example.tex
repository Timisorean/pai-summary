...
\textbf{Jensen's inequality} Let $f : \R^d \rightarrow \R$ be a convex function, $\xx_1, \dots, \xx_n \in \dom(f)$ and $\lambda_1, \dots, \lambda_m \in \R_{+}$ such that $\sum_{i=1}^n{\lambda_i}=1$, then: $f(\sum_{i=1}^m{\lambda_i x\xx_i}) \leq \sum_{i=1}^n{\lambda_i f(\xx_i)}$. \\
