...
\begin{itemize}
    ...
    $\nabla_z \sigma_i(\vf) = \sigma_i(\vf)(1 - \sigma_i(\vf))$ \hfill $\nabla_z \mathrm{ReLU}(z) = \mathbf 1_{\{z \ge 0\}}$
    \item \textbf{Rectified linear unit}: $\mathrm{ReLU}(z) \defeq \max \{z, 0\} \in [0, \infty)$\\[-0.5ex]
\end{itemize}

For linear regression, min. MSE/CE \textbf{corresponds to MLE} under a Gaussian likelihood.
...
\begin{itemize}
    \item BNN learning and inference are \textbf{generally intractable}\\[-0.5ex]
    when the noise is not assumed to be homoscedastic and known. Thus, we need approx. inference.\vspace{-2pt}
    ...
    \item Achieved by max. ELBO with SGD and rep. trick.
    ...
    \scalebox{0.8}{$\begin{aligned}[t]
        \Var{&\ys \mid \vxs, \vx_{1:n}, \vy_{1:n}} \approx \E[\vtheta]{\Var[\ys]{\ys \mid \vxs, \vtheta}} + \Var[\vtheta]{\E[\ys]{\ys \mid \vxs, \vtheta}}\\[-1ex]
...
\begin{itemize}
    \item \textbf{Dropout/Dropconnect} random select/omits vertices/edges of comp. graph. For valid interpretation of this as VI, also need to perform during inference.
    \item Dropout masks will overlap, making preds. highly corr., leading to underest. of epistemic uc. \textbf{Masksembles} mitigate by choosing fixed set of pre-defined masks.
...
\begin{framed}
    \begin{itemize}
        ...
        \vspace{-2pt}
        \item \textbf{MCE:} Max instead of sum
    \end{itemize}
\end{framed}
