...
\begin{itemize}
    ...
    \item \textbf{Inducing pt.}: Summarize data around \scalebox{0.9}{$U \defeq \{\bar\vx_1, ., \bar\vx_n\}$}. Let \scalebox{0.9}{$\vf \defeq \transpose{[f(\vx_1) ., f(\vx_n)]}$} \scalebox{0.9}{$\fs \defeq f(\vxs)$}, \scalebox{0.9}{$\vu \defeq \transpose{[f(\hat\vx_1) \dots f(\hat\vx_n)]}$}. Original GP recoverable with marginalization: \scalebox{0.9}{$p(\fs \mid \vf) = \int_{\R^k} p(\fs, \vf \mid \vu) p(\vu) \,d \vu$}. Approx. joint prior, assuming $\fs$ $\vf$ are cond. indep. given \scalebox{0.9}{$\vu \sim \N{\vzero}{\mK_{UU}}$}\\[-0.2ex]
    ...
    w.\\[-0.5ex]
    \scalebox{0.9}{$\mQ_{ab} \defeq \mK_{aU} \mK_{UU}^{-1} \mK_{Ub}$}. \scalebox{0.9}{$\mK_{AA}$} represents the prior covar. and \scalebox{0.9}{$\mQ_{AA}$} represents covar. from inducing pts.\\
    Covar. mat. comp. is expensive; need to approx.:
    \begin{itemize}
