...
\begin{itemize}
    ...
    \item Gaussian can be repr. using $O(n^2)$ params.; $\Sigma \in \R^{n \times n}$
\end{itemize}
...
\begin{itemize}
    ...
    \item Under some conditions, the Gaussian is self-conjugate (Gaussian prior and likelihood $\rightarrow$ posterior Gaussian).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Improper prior:} It is not required that the prior is a valid distr. (i.e., integrates to 1). We can still derive meaning from the posterior if it's valid.
...
\begin{itemize}
    \item \textbf{Gaussians have maximum entropy among all distr.} with known mean and variance: $\frac12 \ln(e \cdot 2 \pi \sigma^2)$
...
\begin{itemize}
    ...
    \item \textbf{Asymptotically normal} if $\vthetahat_\MLE \convd \N{\opt{\vtheta}}{\mS_n}$ as $n \to \infty$ where $\mS_n$ is the asymptotic covariance of MLE.
...
\scalebox{0.9}{
$\begin{aligned}[t]
\vthetahat_\MAP &\defeq \argmax_{\vtheta \in \Theta} p(\vtheta \mid \vx_{1:n}, y_{1:n}) \\[-1.5ex]
