...
\textbf{$\epsilon$-greedy:} \scalebox{0.7}{\textbullet} Ignores all past experience. \scalebox{0.7}{\textbullet} Will eventually converge. \scalebox{0.7}{\textbullet} Will eventually converge. GLIE with prob. 1 if $(\epsilon_t)_{t \in \mathbb N_0}$ satisfies the RM conditions (e.g., $\epsilon_t = \sfrac1t$).

\vspace{0.2mm}\hrule\vspace{0.2mm}

\textbf{Softmax/Boltzmann exploration:} alt. to $\epsilon$-greedy $\pi_\lambda(a \mid x) \propto \exp(\frac1\lambda Q^\star(x, a))$ (Gibbs). For \scalebox{0.9}{$\lambda \rightarrow 0$} greedily max. Q-fn. For \scalebox{0.9}{$\lambda \rightarrow \infty$} uniform rand. exploration.
...
\begin{itemize}
    \item Optimism in the face of uc. Init. with max reward.
    \item Every $T$ steps, with high prob., either obtains near-\\optimal rew.; or visits one unknown state-action pair.
    \item With prob. at least $1-\delta$, $R_\mathrm{max}$ reaches $\epsilon$-optimal $\pi$ in poly. num. steps $\card{\sX}$, $\card{\sA}$, $T$, $\nicefrac{1}{\epsilon}$, $\nicefrac{1}{\delta}$, and $R_\mathrm{max}$.
\end{itemize}
...
\begin{itemize}
    \item If $\alpha_t$ satisfies RM cond. and all state-action pair are visited inf. often, then $Q^\star$ conv. to $q^\star$ with prob. 1.
    \item With prob. at least $1 - \delta$, conv. to $\epsilon$-optimal policy in num. steps poly. in $\log |X|$, $\log |A|$, $\sfrac 1 \epsilon$ and $\log \sfrac1\delta$.
\end{itemize}

\vspace{0.2mm}\hrule\vspace{0.2mm}

\textbf{Optimistic Q-learning:} Similar to $R_\text{max}$ Init. \scalebox{1}{$Q^\star(x, a) = V_\text{max} \prod_{t=1}^{T_\text{init}} (1 - \alpha_t)^{-1}$} w.\\[-0.5ex] \scalebox{1}{$V_\text{max} \defeq \frac{R_\text{max}}{1 - \gamma} \ge \max q^\star(x, a)$}. With prob. at least $1 - \delta$,\\[-0.8ex] $\epsilon$-optimal $\pi$ after num. steps poly. in $|X|$, $|A|$, $\frac1\epsilon$, $\log \frac1\delta$,\\[-0.5ex]
and $R_\text{max}$ where $T_\text{init}$ is upper bounded by a poly. in same coeff. If $T_\text{init}$ large enough, conv. quickly to $\pi^\star$.