...
\begin{itemize}
    ...
    \item With prob. at least $1 - \delta$, conv. to $\epsilon$-optimal policy in num. steps poly. in $\log |X|$, $\log |A|$, $\frac 1 \epsilon$ and $\log \frac1\delta$.
\end{itemize}

\vspace{0.2mm}\hrule\vspace{0.2mm}

\textbf{Optimistic Q-learning:} Similar to $R_\text{max}$ Init. \scalebox{1}{$Q^\star(x, a) = V_\text{max} \prod_{t=1}^{T_\text{init}} (1 - \alpha_t)^{-1}$} w. \scalebox{1}{$V_\text{max} \defeq \frac{R_\text{max}}{1 - \gamma} \ge \max q^\star(x, a)$}. With prob. at least $1 - \delta$,\\[-0.8ex] $\epsilon$-optimal $\pi$ after num. steps poly. in $|X|$, $|A|$, $\frac1\epsilon$, $\log \frac1\delta$, and $R_\text{max}$ where $T_\text{init}$ is upper bounded by a poly. in same coeff. If $T_\text{init}$ large enough, conv. quickly to $\pi^\star$.