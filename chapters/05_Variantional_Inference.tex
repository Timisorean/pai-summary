...
\vspace{-3.5pt}
Idea: approximate true posterior distribution with a\\[-0.6ex]
simpler posterior that is easy to sample:\\[-0.8ex]
$p(\vtheta \mid \vx_{1:n}, y_{1:n}) = \frac{1}{Z} p(\vtheta, y_{1:n} \mid \vx_{1:n}) \approx q(\vtheta \mid \vlambda) \eqdef q_\vlambda(\vtheta)$, where $\vlambda$ are params. of the \textbf{variational posterior} $q_\vlambda$.

\vspace{0.2mm}\hrule\vspace{0.2mm}

\textbf{Laplace approx.}: find Gaussian approx. (2nd-order\\[-0.5ex]
Taylor) of posterior:
$q(\vtheta) \defeq \N[\vtheta]{\vthetahat}{\inv{\mLambda}} \propto \exp(\hat{\psi}(\vtheta))$,\\[-0.5ex]
with $\vthetahat$ the mode (i.e. MAP estimate) and with $\hes$ the\\[-0.5ex]
Hessian: $\mLambda \defeq - \hes_\psi(\vthetahat) = - \hes_\vtheta \log p(\vtheta \mid \vx_{1:n}, y_{1:n}) \bigl|_{\vtheta = \vthetahat}$. \\[-0.5ex]
...
\begin{itemize}
    ...
    \item Preserves MAP point estimate as its mean.
    \item Laplace approximation of a Gaussian is exact.\vspace{-2pt}
    \item \textbf{For log. BLR:} $\mathbf{\Lambda} = \mathbf{X}^\top \text{diag} \{ \pi_i (1 - \pi_i) \} \mathbf{X} + \sigma_{\text{p}}^{-2} \mathbf{I}$\\[-0.5ex]
    (indep. of norm.const.) $p(\ys \mid \vxs, \vx_{1:n}, y_{1:n})$\\[-0.5ex]
    \scalebox{0.9}{
    $\begin{aligned}
    &\approx \smallint \sigma(\ys \transpose\vw \vx) \mathcal{N}(\vw; \hat\vw, \Lambda^{-1}) \,d \vw \\[-1.2ex]
    &= \smallint \sigma(\ys f) \mathcal{N}(f; \hat{\mathbf{w}}^T \mathbf{x}^*, \mathbf{x}^{*T} \Lambda^{-1} \mathbf{x}^*) \,d f
    \end{aligned}$}
\end{itemize}
...
\begin{framed}
    ...
    \begin{itemize}
        \item $\S{u} > \S{v} \implies u < v$ (anti-monoton) \scalebox{0.7}{\textbullet} $S$ continuous
...
\begin{itemize}
    ...
    \item ELBO gradient is gen. \textbf{intractable} (use rep. trick).
\end{itemize}

\begin{framed}
    \textbf{Repearm. trick:} Let $\vepsilon \sim \phi$ be indep. of $\vlambda$, $\vg : \R^d \to \R^d$ be a diff. and inv. fn, $\vtheta \defeq \vg(\vepsilon; \vlambda)$, and $\vf$ a \textit{nice} fn. We get: \scalebox{0.83}{$
    q_\vlambda(\vtheta) = \phi(\vepsilon) \cdot \inv{\abs{\det{\jac_\vepsilon \vg(\vepsilon; \vlambda)}}} $}; \hfill \scalebox{0.83}{$\E[\vtheta \sim q_\vlambda]{\vf(\vtheta)} = \E[\vepsilon \sim \phi]{\vf(\vg(\vepsilon; \vlambda))}
    $}
\end{framed}

\begin{itemize}
    \item For ELBO $\grad_\vlambda \E[\vtheta \sim q_\vlambda]{\vf(\vtheta)} = \E[\vepsilon \sim \phi]{\grad_\vlambda \vf(\vg(\vepsilon; \vlambda))}$.
     % If we can find $\vg$ and suitable reference density $\phi$ which is indep. of $\vlambda$, we say $q_\vlambda$ is \textbf{reparametrizable}.
