...
\end{framed}
\begin{framed}
    \textbf{KL-divergence}: measures the additional expected surprise when observing samples from $p$ that is due to assuming the (wrong) distribution $q$ and which is not inherent in the distribution $p$ already.\\[-0.5ex]
    $\KL{p}{q} \defeq \crH{p}{q} - \H{p} = \E[\vtheta \sim p]{\log \frac{p(\vtheta)}{q(\vtheta)}}$\\
    Properties:
    \begin{itemize}
        \item $\KL{p}{q} \geq 0$ 
        \item $\KL{p}{q} = 0$ iff $p = q$ almost surely 
...
\begin{itemize}
    ...
    \item For ${p \defeq \N{\vmu_p}{\mSigma_p}}$ and ${q \defeq \N{\vmu_q}{\mSigma_q}}$: \scalebox{0.9}{$\begin{aligned}[t]
    \KL{p}{q} = \frac{1}{2} (\mathrm{tr}(\inv{\mSigma_q} \mSigma_p) + \transpose{(\vmu_p - \vmu_q)} &\inv{\mSigma_q} (\vmu_p - \vmu_q) \phantom{\frac12}\\[-1.4ex]
    &- d + \log \frac{\det{\mSigma_q}}{\det{\mSigma_p}})\end{aligned}$}
\end{itemize}

\vspace{0.5mm}\hrule\vspace{0.5mm}

\begin{itemize}
    \item \textbf{Forward KL}: $\qs_1 \defeq \argmin_{q \in \spQ} \KL{p}{q}$\\[-0.2ex]
    (more conservative and yields more “desired” approx.)
    \item \textbf{Reverse KL}: $\qs_2 \defeq \argmin_{q \in \spQ} \KL{q}{p}$ (greedily\\[-0.2ex]
    selects mode, underestimate var., overconfident preds.)
\end{itemize}
...
\begin{itemize}
    \item Max. ELBO selects a variational distr. $q$ that is close to prior distr. $p(\cdot)$ while also max. avg. likelihood of data $p(y_{1:n} \mid \vx_{1:n}, \vtheta)$ for $\vtheta \sim q$. This is in contrast to MAP estimation, which picks single mode $\bm \theta$ that max. the likelihood and proximity to the prior.
