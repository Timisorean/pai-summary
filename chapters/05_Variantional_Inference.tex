...
\begin{itemize}
    ...
    \KL{p}{q} = \frac{1}{2} (\mathrm{tr}(\inv{\mSigma_q} \mSigma_p) + \transpose{(\vmu_p - \vmu_q)} &\inv{\mSigma_q} (\vmu_p - \vmu_q) \phantom{\frac12}\\[-1.6ex]
...
\begin{itemize}
    \item Max. ELBO selects a variational distr. $q$ that is close to prior distr. $p(\cdot)$ while also max. avg. likelihood of data $p(y_{1:n} \mid \vx_{1:n}, \vtheta)$ for $\vtheta \sim q$. This is in contrast to MAP estimation, which picks single mode $\vtheta$ that max. the likelihood and proximity to the prior.
