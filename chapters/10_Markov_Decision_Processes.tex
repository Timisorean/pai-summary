...
\vspace*{-2pt}
\begin{framed}
    \item \textbf{Discounted payoff}: $G_t \defeq \sum_{m=0}^\infty \gamma^m R_{t+m}$
\end{framed}
...
Optimal $v^\star$ and $q^\star$ are fixed-pt. of Bellman update.

\begin{framed}
    \textbf{Bellman optimality equations:}\\
    $\v*{x} = \max_{a \in A} r(x,a) + \gamma \E[x' \mid x,a]{\v*{x'}}$\\[-0.5ex]
    $\q*{x}{a} = r(x,a) + \gamma \E[x' \mid x,a]{\max_{a \in A} \q*{x'}{a'}}$
\end{framed}
...
\begin{tabular}{@{}p{0.55\columnwidth}@{\extracolsep{\fill}}p{0.45\columnwidth}@{}}
    ...
    \begin{itemize}
        ...
        \item  For finite MDPs, PI converges to $\pi^\star$ in poly. num. of iter. Each step takes cubic comp. in the num. of states.
    \end{itemize}& 
...
\begin{itemize}
    ...
    \item \scalebox{0.85}{$v^{\pi_{t+1}}(x) >v^{\pi_{t}}(x)$} for at least one $x \in X$, unless \scalebox{0.8}{$v^{\pi_t} \equiv v^\star$}
\end{itemize}

\vspace{0.2mm}\hrule\vspace{0.2mm}
\includegraphics[width=0.8\linewidth,trim={0 0.6cm 0 0.8cm},clip]{images/Value_Iteration.png}
\vspace{-3pt}
...
\begin{itemize}
    ...
    \item VI can be viewed from dynamic prog. perspective
\end{itemize}

\begin{framed}
    POMDP: Markov process, with \textbf{observations} $\sY$, and \textbf{observation probs.} \scalebox{0.9}{$o(y \mid x) \defeq \Pr{Y_t = y \mid X_t = x}$}. Hard to solve in gen., can conv. to MDP with larger state space.
\end{framed}
...
%     \item The seq. of belief-states defines the seq. of RVs \scalebox{0.9}{$(B_t)_{t\in\Nat_0}$:$ B_t \defeq X_t \mid y_{1:t}, a_{1:t-1}$}, where the (state-)space of all beliefs is the (infinite) space of all prob. distr. over $\sX$: \scalebox{0.75}{$\spB \defeq \Delta^{\sX} \defeq \braces*{\vb \in \R^{\card{\sX}} : \vb \geq \vzero, \textstyle\sum_{i=1}^{\card{\sX}} \vb(i) = 1}$}.
% \end{itemize}
