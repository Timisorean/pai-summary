...
\begin{framed}
    \textbf{Cumulative regret} associated with choices $\{\vx_t\}_{t=1}^T$ is: $R_T \defeq \sum_{t=1}^T \underbrace{({\max_\vx \opt{f}(\vx) - \opt{f}(\vx_t)})}_{\text{\textit{instantaneous regret}}}$.
\end{framed}
...
\begin{framed}
    When choosing $\beta_t$ appropriately: \scalebox{0.8}{$R_T = \BigO{\sqrt{T \gamma_T}}$}, with \scalebox{0.9}{$\gamma_T \defeq \max_{\substack{\sS \subseteq \spX \\ \card{\sS} = T}} \I{\vf_\sS}{\vy_\sS} = \max_{\substack{\sS \subseteq \spX \\ |\sS| = T}} \frac{1}{2} \log\det{\mI + \sigman^{-2} \mK_{\sS\sS}}$}, is the maximum information gain after $T$ rounds.
    \begin{itemize}
