...
\begin{framed}
    ...
    \begin{itemize}
        \item $\I{\rX}{\rY} = \I{\rY}{\rX}$
        \item $\I{\rX}{\rY} = \E[\vy \sim p]{\KL{p(\vx \mid \vy)}{p(\vx)}}$
    \end{itemize}
    \vspace{0.5mm}\hrule\vspace{0.5mm}
    ...
    \begin{itemize}
        ...
        \item For Gaussian $\I{\rX}{\rY} = \frac12 \log \det{\mI + \sigma_n^{-2} \mSigma)}$
    \end{itemize}
\end{framed}

\begin{framed}
    \begin{itemize}
        \item Given (discrete) fn. $F : \pset{\spX} \to \R$, the \textbf{marginal gain} of $\vx \in \spX$ given $\sA \subseteq \spX$ is: \scalebox{0.9}{$\Delta_F(\vx \mid \sA) \defeq F(\sA \cup \{\vx\}) - F(\sA)$}.
...
\begin{itemize}
    \item \textbf{Greedy}: Pick the locations $\vx_1$ through $\vx_n$ individually by greedily finding the location with the maximal MI, this provides a $(1 - \nicefrac{1}{e})$-approximation of the optimum. 
